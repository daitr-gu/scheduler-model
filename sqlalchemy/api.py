...
def partners_get_all(context):
    return model_query(context, models.Partner).all()


def partners_get_by_shortname(context, shortname):
    return model_query(context, models.Partner).\
                    filter_by(shortname=shortname).\
                    first()
...
def partners_update(context, shortname, values):
    session = get_session()
    with session.begin():
        partner = partners_get_by_shortname(context, shortname)
        if not partner:
            return False
...
def temp_instances_get_by_host(context, host):
    return model_query(context, models.TempInstances).\
